...
import { TextField, Button, RadioGroup, FormControlLabel, Radio } from '@mui/material'; // Importez les composants nécessaires depuis @mui/material
import * as Yup from "yup";
import { useNavigate } from 'react-router-dom';

const initialValues = { firstName: "", lastName: "", email: "", password: "", gender: "" }; // Ajoutez la clé gender à initialValues
const validationSchema = Yup.object().shape({
  firstName: Yup.string().required("First Name is required"),
  lastName: Yup.string().required("Last Name is required"),
  email: Yup.string().email("Invalid email").required("Email is required"),
  password: Yup.string().min(6, "Password must be at least 6 characters").required("Password is required"),
  gender: Yup.string().required("Gender is required"), // Ajoutez une validation pour le genre
});
  ...
  return (
    <>
      <Formik validationSchema={validationSchema} initialValues={initialValues} onSubmit={handleSubmit}>
        {({ values, handleChange }) => ( // Utilisez les valeurs et la fonction handleChange fournies par Formik
